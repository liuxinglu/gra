...
var app;
(function (app) {
    var WuJiaoView = (function (_super) {
        __extends(WuJiaoView, _super);
        function WuJiaoView() {
            return _super.call(this, lxl.Config.SKIN_PATH + "WuJiaoViewSkin.exml") || this;
        }
        WuJiaoView.prototype.onActivity = function () {
            _super.prototype.onActivity.call(this);
            this._shape = new egret.Shape();
            this.group.addChild(this._shape);
            this.drawBaseLine();
        };
        WuJiaoView.prototype.updateView = function () {
        ...
        WuJiaoView.prototype.drawBaseLine = function () {
        ...
        WuJiaoView.prototype.getEachPoint = function () {
            ...
            for (var i = 0; i < Gra.ablities.length; i++) {
                var s = [];
                s.push(Gra.getTotalCountByAblityName(Gra.ablities[i]));
                s.push(Gra.getAveCountByAbilityName(Gra.ablities[i]));
                s.push(Gra.getCountByAblityName(Gra.curName, Gra.ablities[i], Gra.curId));
                arr.push(s);
                this["lab_" + i].text = Gra.ablities[i];
            }
            var scorepArr = [];
            var avepArr = [];
            var px = 0;
            var py = 0;
            for (var i = 0; i < 5; i++) {
                this["unit" + i] = this["l" + i] / arr[i][0];
                if (i < 3 && i != 0) {
                    this["al" + i] = this["unit" + i] * arr[i][1];
                    this["rl" + i] = this["unit" + i] * arr[i][2];
                    px = this.p_c.x + (Math.abs(this["p_" + i].x - this.p_c.x) * (this["rl" + i] / this["l" + i]));
                }
                else {
                    this["al" + i] = this["l" + i] - this["unit" + i] * arr[i][1];
                    this["rl" + i] = this["l" + i] - this["unit" + i] * arr[i][2];
    ...
    app.WuJiaoView = WuJiaoView;
    __reflect(WuJiaoView.prototype, "app.WuJiaoView");
})(app || (app = {}));
//# sourceMappingURL=WuJiaoView.js.map